...
| `FITBIT_CLIENT_ID` / `FITBIT_CLIENT_SECRET` | Direct Fitbit activity, heart-rate, and sleep sync | Fitbit developer app | Add the callback `<your-domain>/api/connectors/fitbit/callback` and enable `activity heartrate sleep profile` scopes. |
| `OURA_CLIENT_ID` / `OURA_CLIENT_SECRET` | Direct Oura activity, sleep, HRV, and readiness sync | Oura developer app | Add the callback `<your-domain>/api/connectors/oura/callback` and enable `daily personal session` scopes. |
| `APNS_KEY_ID`, `APPLE_TEAM_ID`, `APNS_AUTH_KEY` | TestFlight/App Store iPhone push notifications | Apple Developer Program | Set `APNS_AUTH_KEY` to the contents of the APNs Auth Key `.p8` file, preserving newlines (or encoding them as `\n`). `APNS_BUNDLE_ID` defaults to `com.functioningfaith.app`; set `APNS_ENV=sandbox` only for debug builds. |
