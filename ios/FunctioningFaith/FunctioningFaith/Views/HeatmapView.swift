            ...
            Map(position: $position) {
                ForEach(map?.cells ?? []) { cell in
                    Annotation("\(cell.count) activities", coordinate: CLLocationCoordinate2D(latitude: cell.latitude, longitude: cell.longitude)) {
                        Circle().fill(FFTheme.hearth.opacity(min(0.9, 0.2 + Double(cell.count) / 10))).frame(width: max(12, min(42, 10 + CGFloat(cell.count) * 4)), height: max(12, min(42, 10 + CGFloat(cell.count) * 4))).overlay(Circle().stroke(.white.opacity(0.7), lineWidth: 1))
                    }
                }
            }
